...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "custom_components" / "threed_ble_map"))
...
def simulate(penalties: dict[tuple[str, str], float] | None = None):
    ...
    for advertiser in ADVERTISERS:
        for listener in TRUTH:
            ...
            direct[(listener, advertiser)] = [rssi(distance) - penalty]
    ...
    for index in range(BEACON_COUNT):
        ...
        for anchor, position in TRUTH.items():
            reading = rssi(math.dist(position, beacon))
...
def main() -> int:
    passed = True
    for test in (test_recovers_shape, test_rejects_attenuated_link, test_edge_cases):
        print(f"\n{test.__name__}")
        passed &= test()
