...
def _beacon_map(result) -> dict:
    return {
        b["address"]: {axis: b[axis] for axis in "xyz"}
        for b in (result.get("beacons") or [])
        if "x" in b
    }


def test_tracking() -> bool:
    """Beacons must move between full solves without the map drifting.

    The full solve is the yardstick here, not the truth: this path claims to
    reproduce what the joint solve would have said, for a fraction of the cost.
    Whether the joint solve is itself right is what the other tests measure.
    """
    import time
    ...
    joint = _beacon_map(solved)

    started = time.perf_counter()
    tracked = tracking.track(frame, observations, joint)
    cost_ms = (time.perf_counter() - started) * 1000

    drift = sorted(
        math.dist(
            [joint[a][k] for k in "xyz"], [tracked[a][k] for k in "xyz"]
        )
        for a in tracked
        if a in joint
    )
    ok = check(
        "tracking reproduces the joint solve's beacons",
        drift[len(drift) // 2] < 1.0,
        f"median {drift[len(drift)//2]:.2f} m, p90 {drift[int(len(drift)*0.9)]:.2f} m "
        f"over {len(drift)} beacons",
    )
    ok &= check(
        "and does it in milliseconds",
        cost_ms < 250,
        f"{cost_ms:.1f} ms for {len(tracked)} beacons",
    )

    # Nothing changed, so nothing should move. A beacon the geometry barely
    # determines will always jitter; what matters is that the typical one does
    # not wander off while the house sits still.
    current = dict(tracked)
    for _ in range(15):
        current = tracking.track(frame, observations, current)
    settled = sorted(
        math.dist([tracked[a][k] for k in "xyz"], [current[a][k] for k in "xyz"])
        for a in current
        if a in tracked
    )
    ok &= check(
        "an unchanged house holds its beacons still",
        settled[len(settled) // 2] < 1.0,
        f"median {settled[len(settled)//2]:.2f} m after 15 updates",
    )

    # Now actually move one, by regenerating its readings from a new position.
    # Move one beacon and check it is followed. The destination has to be
    # named in the solved frame, not in truth coordinates: MDS returns a layout
    # congruent to the house but arbitrarily rotated and reflected, so "6, 3, 1"
    # means nothing here. Readings are generated by inverting the same
    # range-to-RSSI law the tracker inverts, which is what makes this a test of
    # whether it follows rather than a test of the path-loss model.
    mover = max(
        (a for a in current),
        key=lambda a: sum(1 for anchor in TRUTH if a in observations[anchor]),
    )
    radios = frame["radios"]
    moved_to = [
        sum(radios[a][axis] for a in anchors) / len(anchors) + offset
        for axis, offset in zip("xyz", (2.0, -1.5, 0.5))
    ]
    bias = refine.shadowing_bias(frame["shadowing_db"])
    shifted = {a: dict(readings) for a, readings in observations.items()}
    for anchor in anchors:
        distance = max(
            math.dist([radios[anchor][k] for k in "xyz"], moved_to), 0.3
        )
        shifted[anchor][mover] = (
            geometry.TX_POWER_AT_1M
            - 10 * geometry.PATH_LOSS_EXPONENT * math.log10(distance * bias)
            + frame["gains"].get(anchor, 0.0)
        )
    before = math.dist([current[mover][k] for k in "xyz"], moved_to)
    following = dict(current)
    for _ in range(10):
        following = tracking.track(frame, shifted, following)
    after = math.dist([following[mover][k] for k in "xyz"], moved_to)
    ok &= check(
        "a beacon that moves is followed there",
        after < 1.5 and after < before,
        f"{before:.2f} m away when it moved, {after:.2f} m after 10 updates",
    )

    others = sorted(
        math.dist([current[a][k] for k in "xyz"], [following[a][k] for k in "xyz"])
        for a in following
        if a in current and a != mover
    )
    ok &= check(
        "and the rest of the map does not follow it",
        others[len(others) // 2] < 0.5,
        f"median {others[len(others)//2]:.2f} m moved by the other "
        f"{len(others)} beacons",
    )
    return ok
